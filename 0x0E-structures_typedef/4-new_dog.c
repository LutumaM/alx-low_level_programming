#include "dog.h"
#include <stdlib.h>
int _strlen(char *s);
char *_strcpy(char *x, char *y);

/**
 * new_dog - makes a new dog, memor and all
 * @name: name to create mem and assign for dog
 * @age: age to assign to dog
 * @owner: owne to create mem and assign to dog
 *
 * Return: pointer to new_dog_t (struct dog)
 */
dog_t *new_dog(char *name, float age, char *owner)
{
	dog_t *new_d;

	new_d = malloc(sizeof(dog_t));
	if (new_d == NULL)
	{
		return (NULL);
	}
	if (name == NULL)
	{
		new_d->name = NULL;
	}
	else
	{
		new_d->name = malloc(_strlen(name) + 1);
		if (new_d->name == NULL)
		{
			free(new_d);
			return (NULL);
		}
		new_d->name = _strcpy(new_d->name, name);
	}

	if (owner == NULL)
	{
		new_d->owner = NULL;
	}
	else
	{
		new_d->owner = malloc(_strlen(owner) + 1);
		if (new_d->owner == NULL)
		{
			free(new_d->name);
			free(new_d);
			return (NULL);
		}
		new_d->owner = _strcpy(new_d->owner, owner);
	}
	new_d->age = age;
	return (new_d);
}

/**
 * _strlen - gets length of a string
 * @s: string to get length of
 *
 * Return: length of the string
 */
int _strlen(char *s)
{
	int i = 0;

	for (i = 0; s[i]; i++)
		;
	return (i);
}

/**
 * _strcpy - copies string from y to x
 * @x: pointer to dest of string
 * @y: pointer to src of string
 *
 * Return: pointer to dest
 */
char *_strcpy(char *x, char *y)
{
	char *a = x;

	while (*y)
		*a++ = *y++;
	*a = '\0';
	return (x);
}
